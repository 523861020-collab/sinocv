    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex flex-col items-center justify-center" onClick={onClose} style={{background: 'transparent'}}>
      ...
      {images.length > 1 && (
        <div className="mt-4 flex items-center gap-4 z-50">
          ...
          <div className="flex gap-2">{images.map((_, i) => <div key={i} className={`h-2 w-2 rounded-full ${i === idx ? 'bg-blue-500' : 'bg-white/40'}`} />)}</div>
...
function ZoneWindow({ zone }: { zone: typeof zones[0] }) {
  ...
  return (
    <div className="rounded-2xl bg-gray-900 border border-gray-800 overflow-hidden group hover:border-blue-500/40 transition-colors">
      ...
      <div className="relative h-52 overflow-hidden bg-gray-800">
        ...
        <div className="absolute top-3 left-3 rounded-full bg-blue-500 px-2.5 py-0.5 text-xs font-semibold text-black">
      ...
      <div className="flex justify-center gap-1 pb-3">
        {products.map((_, i) => (
          <div key={i} className={`h-1 rounded-full transition-all ${i === current ? 'w-4 bg-blue-500' : 'w-1.5 bg-gray-700'}`} />
        ))}
      </div>
    </div>
  );
          ...
          <div className="text-center">
            ...
            <div className="flex flex-col items-center">
              <div className="relative mb-8">
                ...
                <img src="/images/manager.jpg" alt="Li Shanlong" className="relative w-56 h-56 rounded-full object-cover border-4 border-blue-500/40 shadow-2xl" />
              </div>
              <h4 className="text-white font-bold text-5xl mb-3">Li Shanlong</h4>
              <p className="text-blue-400 text-xl font-medium mb-6">General Manager</p>
            ...
            <p className="text-gray-300 leading-relaxed text-base text-left">
              <span className="text-blue-400 font-semibold">SINOCV</span> is a Hong Kong-based professional commercial vehicle and construction machinery export company, providing full-chain export services from vehicle selection, customization, inspection, logistics, to customs clearance.
            </p>
            <p className="text-gray-300 leading-relaxed text-base text-left">
            ...
            <p className="mt-12 pt-8 border-t border-gray-800/50 text-gray-400 italic text-base text-left">
              🎯 <span className="text-blue-400 font-medium">Our Mission:</span> Customer-focused, quality-driven — to provide the finest Chinese commercial vehicles and construction machinery to global customers and become a trusted long-term partner.
            </p>
            <h3 style={{marginTop: '2rem'}} className="text-2xl font-bold text-white mb-8">Certifications</h3>
            <div className="flex flex-wrap justify-center gap-6">
              <a href="/images/cert-hk-1.jpg" target="_blank" className="rounded-xl overflow-hidden border border-gray-800 hover:border-blue-500/40 transition-all block bg-gray-900 w-full max-w-sm">
              ...
              <a href="/images/cert-hk-2.jpg" target="_blank" className="rounded-xl overflow-hidden border border-gray-800 hover:border-blue-500/40 transition-all block bg-gray-900 w-full max-w-sm">
